	...
	case WM_COMMAND:		//ボタンが押された時
	   ...
	   ********/
	    ...
	    ***********************/
		...
		//ここから
		case IDC_RADIO2:	//ラジオボタン（赤）
			color = RGB(0, 255, 0);				//色指定
			InvalidateRect(hWnd, NULL, TRUE);	//再描画
			return TRUE;

		case IDC_RADIO3:	//ラジオボタン（赤）
...
HRESULT CALLBACK WndProc( HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam )
{
	...
	case WM_PAINT:
		...
		********************************/
		...
		LineTo(hdc, 130, 50);
		MoveToEx(hdc, 85, 70, NULL);
		LineTo(hdc, 150, 70);
		LineTo(hdc, 115, 100);			//ラ
		MoveToEx(hdc, 210, 50, NULL);
		LineTo(hdc, 170, 70);
		MoveToEx(hdc, 190, 60, NULL);
		LineTo(hdc, 190, 100);			//イ
		MoveToEx(hdc, 100, 120, NULL);
		LineTo(hdc, 130, 120);
		MoveToEx(hdc, 90, 140, NULL);
		LineTo(hdc, 150, 140);
		MoveToEx(hdc, 120, 140, NULL);
		LineTo(hdc, 100, 170);
		MoveToEx(hdc, 140, 115, NULL);
		LineTo(hdc, 150, 120);
		MoveToEx(hdc, 150, 115, NULL);
		LineTo(hdc, 160, 120);			//デ
		MoveToEx(hdc, 165, 120, NULL);
		LineTo(hdc, 210, 120);
		LineTo(hdc, 165, 170);
		MoveToEx(hdc, 187, 145, NULL);
		LineTo(hdc, 210, 170);			//ス
		MoveToEx(hdc, 220, 120, NULL);
		LineTo(hdc, 260, 120);
		LineTo(hdc, 260, 170);
		LineTo(hdc, 220, 170);			//コ
		MoveToEx(hdc, 270, 120, NULL);
		LineTo(hdc, 270, 170);
		LineTo(hdc, 300, 145);
		...
		TextOut(hdc, 10, 10, TEXT("Tsurai"),6);		//テキスト描画
